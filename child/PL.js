...
function initEmbedFeed() {
  const root = document.querySelector("#embed-feed");
  if (!root) return;

  // Lấy đường dẫn JSON từ data-embed-src (mặc định: PL-embed.json)
  const src = root.dataset.embedSrc || "PL-embed.json";
  const bust = `v=${Date.now()}`; // ép bỏ cache khi bạn cập nhật file JSON
  const url = src.includes("?") ? `${src}&${bust}` : `${src}?${bust}`;

  // Skeleton trong lúc tải
  root.innerHTML = `
    <div class="embed-skeleton">
      <div class="embed-skel-card"></div>
      <div class="embed-skel-card"></div>
      <div class="embed-skel-card"></div>
    </div>`;
  ...
  const toEmbedURL = (u) => {
    try {
      ...
      if (
        ...
        return `https://www.youtube.com/embed/${x.searchParams.get("v")}`;
      }
      if (/youtu\.be$/.test(h) && x.pathname.length > 1) {
        return `https://www.youtube.com/embed/${x.pathname.slice(1)}`;
      }
      return x.href;
    } catch {
      return u;
  ...
  fetch(url, { cache: "no-store" })
    ...
    .then((data) => {
      ...
      if (!items.length) {
        root.innerHTML = `<div class="embed-empty">Chưa có nội dung.</div>`;
        return;
      }
      const allow =
      ...
      root.innerHTML = items
        .map((it, i) => {
          const title = esc(it.title || `Nội dung #${i + 1}`);
          const raw = (it.url || it.link || "").trim();
          const src = toEmbedURL(raw);
          const open = raw
            ? ` href="${esc(raw)}" target="_blank" rel="noopener"`
            : ` href="#"`;
          return `
<article class="embed-card">
  <header class="embed-card__head">
    <h2 class="embed-card__title">${title}</h2>
    <a class="btn-primary btn-pill embed-card__open"${open} aria-label="Mở '${title}'">Mở trang</a>
  </header>
  <div class="embed-card__frame">
    ...
    .catch((e) => {
      console.warn("Embed feed error:", e);
      root.innerHTML = `<div class="embed-error">Không tải được dữ liệu. Kiểm tra file JSON.</div>`;
    });
}
document.addEventListener("DOMContentLoaded", () => {
  // … các init khác …
