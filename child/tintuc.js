...
/* ===== NEWS GRID VIP PRO (data-driven từ news.json) ===== */
(function NewsSystem() {
  const SEL = {
    host: ".news-system",
    tabs: ".news-system .news-tab",
    track: ".news-system .news-track",
    navPrev: ".news-system .news-nav.prev",
    navNext: ".news-system .news-nav.next",
    dots: ".news-system .news-dots",
  };
  const REFRESH_MS = 20000; // 20s: tự kiểm tra thay đổi file json
  const state = {
    all: [],
    currentCat: "all",
    sig: "",
  ...
  function hash(obj) {
    try {
      return (
        String(obj.length) +
        ":" +
        btoa(unescape(encodeURIComponent(JSON.stringify(obj)))).slice(-24)
      );
    } catch (_) {
  ...
  async function loadNews(src) {
    try {
      ...
      const list = Array.isArray(json?.news) ? json.news : [];
      return list;
    } catch (e) {
      console.error("[NewsSystem] load error:", e);
      return [];
    }
  }

  // Thay thế hoàn toàn hàm buildCard cũ
  function buildCard(n) {
    const id = String(n.id || "").trim();
    const detailUrl = `tintuc(1).html?id=${encodeURIComponent(id)}`;
    ...
    <div class="body">
      ...
      <a class="more" href="${detailUrl}">Xem thêm</a>
    </div>
  `;
    return article;
  }
  ...
  function render(cat) {
    ...
    const dotsWrap = $(SEL.dots);
    if (!track) return;

    const data = state.all.filter((n) => {
    ...
    data.forEach((n) => frag.appendChild(buildCard(n)));
    track.appendChild(frag);

    // Reset scroll & nav visibility
    track.scrollTo({ left: 0, behavior: "instant" });
    requestAnimationFrame(() => updateNavVisibility());

    // Rebuild dots (optional)
    // Dots click -> go to page (jump instantly)
    if (dotsWrap) {
      dotsWrap.addEventListener("click", (e) => {
        const dot = e.target.closest(".news-dot");
        if (!dot) return;
        const idx = parseInt(dot.getAttribute("data-i"), 10);
        if (isFinite(idx)) {
          // xác định hướng để cập nhật mũi tên
          lastDir = idx > currentPage() ? 1 : -1;
          updateNavVisibility(lastDir);

          // tắt smooth 1 nhịp, cuộn ngay lập tức
          const old = track.style.scrollBehavior;
          track.style.scrollBehavior = "auto";
          track.scrollTo({ left: idx * track.clientWidth, behavior: "auto" });
          // trả lại smooth sau khi đã nhảy
          requestAnimationFrame(() => {
  ...
  function updateNavVisibility() {
    const track = $(SEL.track);
    const prev = $(SEL.navPrev);
    const next = $(SEL.navNext);
    ...
    if (dotsWrap) {
      ...
      const page = Math.round(track.scrollLeft / (track.clientWidth + 14));
      const active = Math.min(
        dotsWrap.children.length - 1,
        Math.floor(
          (track.scrollLeft / track.scrollWidth) *
            Math.ceil(track.children.length / per)
        )
      );
      $all(".news-dot", dotsWrap).forEach((el, i) =>
  ...
  function bindTabs() {
    $all(SEL.tabs).forEach((btn) => {
      btn.addEventListener("click", () => {
        $all(SEL.tabs).forEach((b) => {
        ...
        render(state.currentCat);
      });
    });
  }

  // Dán toàn bộ hàm này để thay thế cho hàm bindNav() cũ
  // Thay toàn bộ bindNav() trong tintuc.js bằng hàm này
  function bindNav() {
    const track = $(SEL.track);
    const prev = $(SEL.navPrev);
    const next = $(SEL.navNext);
    ...
    function step(dir) {
      const delta = Math.max(200, Math.round(track.clientWidth * 0.92));
      track.scrollBy({ left: dir * delta, behavior: "smooth" });
    }
    prev.addEventListener("click", () => step(-1));
    next.addEventListener("click", () => step(1));

    // --- Kéo để cuộn nhưng không chặn click ---
    let isDown = false;
    let startX = 0;
    let startLeft = 0;
    let dragged = false;
    let captured = false;
    const THRESH = 6; // px – di chuyển vượt ngưỡng mới tính là kéo
    ...
    track.addEventListener("pointerdown", (e) => {
      ...
      track.style.scrollBehavior = "auto"; // tắt smooth khi đang kéo
      // KHÔNG setPointerCapture ở đây!
    });
    ...
    track.addEventListener("pointermove", (e) => {
      ...
      // Chỉ khi vượt ngưỡng mới coi là kéo và bắt pointer
      if (!dragged && Math.abs(dx) > THRESH) {
        dragged = true;
        try {
    ...
    const onPointerUp = (e) => {
      ...
      if (captured && track.hasPointerCapture?.(e.pointerId)) {
        try {
          track.releasePointerCapture(e.pointerId);
        } catch (_) {}
      }

      // Nếu vừa kéo, chặn cú click tổng hợp ngay sau đó để không mở link ngoài ý muốn
      if (dragged) {
        const blockClick = (ev) => {
          ev.preventDefault();
          ev.stopPropagation();
        };
        track.addEventListener("click", blockClick, {
          capture: true,
          once: true,
        });
      }
  ...
  async function refreshIfChanged(src) {
    const list = await loadNews(src);
    const sig = hash(list);
    if (sig !== state.sig) {
      state.sig = sig;
      state.all = list;
  ...
  async function init() {
    const host = $(SEL.host);
    if (!host) return;
    const src = host.getAttribute("data-news-src") || "news.json";

    // Lần đầu
    await refreshIfChanged(src);

    // Tabs + Nav
    bindTabs();
    bindNav();

    // Tự cập nhật khi file json thay đổi (polling nhẹ)
    setInterval(() => refreshIfChanged(src), REFRESH_MS);
  }

  // Khởi chạy sau khi DOM sẵn sàng; vẫn tách biệt với các init khác.
